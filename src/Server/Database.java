...
public class Database {
    ...
    boolean joinCheck(String _n, String _nn, String _i, String _p, String _e) {
        ...
        try {
            //member 테이블에 각 문자열들을 순서대로 업데이트하는 문장. 0승 0패, 1000점 으로 초기화
    ...
    //아이디 or 닉네임 중복 체크 메소드! 중복값 존재o → false, 존재x → true를 반환한다.
    boolean overCheck(String _a, String _v) {
        boolean flag = false;	//참거짓을 반환할 flag 변수. 초기값은 false

        //att(attribute)는 속성(아이디, 닉네임)을 구분하고, val(value)은 확인할 값이 초기화.
        String att = _a;
        String val = _v;
